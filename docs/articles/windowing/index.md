...
- **State-scoped lifecycle**: `IWindowManager` is state-scoped (via `[StateFacade]`), meaning windows have the same lifecycle as the game state they belong to.
- **Event polling**: Call `PollEvents()` once per frame to process window system events before reading input state.

## Topics

- **<xref:sparkitect.windowing.windowing-input>** - Window management, keyboard/mouse input, Vulkan surface integration
