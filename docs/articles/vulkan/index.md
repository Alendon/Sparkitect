...
- **Result error handling**: object creation returns [`Result<T, VkApiResult>`](xref:Sparkitect.Utils.DU.Result`2), a discriminated union that forces explicit error handling through pattern matching. There are no silent failures.
...
- **<xref:sparkitect.vulkan.vulkan-graphics>** - [`IVulkanContext`](xref:Sparkitect.Graphics.Vulkan.IVulkanContext), wrapper types, `Result<T, VkApiResult>` handling, CallerContext, resource lifecycle
