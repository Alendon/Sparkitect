...
public class StateModuleServiceGeneratorTests : SourceGeneratorTestBase<StateModuleServiceGenerator>
{
    ...
    [Test]
    public async Task ExtractStateServiceInfo_ValidService_ExtractsCorrectly(CancellationToken token)
    {
        TestSources.Add(("TestService.cs",
            ...
            [StateService<ITestService, TestModule>]
            public class TestService : ITestService { }
            """));

        var (_, compilation) = await GetInitialCompilationAsync(token);
        var type = compilation.GetTypeByMetadataName("StateServiceTest.TestService");

        await Assert.That(type).IsNotNull();

        var serviceInfo = StateModuleServiceGenerator.ExtractStateServiceInfo(type!);

        await Assert.That(serviceInfo).IsNotNull();
        await Assert.That(serviceInfo!.ModuleTypeFullName).IsEqualTo("global::StateServiceTest.TestModule");
        await Assert.That(serviceInfo.ModuleTypeName).IsEqualTo("TestModule");
        await Assert.That(serviceInfo.FactoryTypeName).IsEqualTo("global::StateServiceTest.TestService_Factory");
    }

    [Test]
    public async Task GroupServicesByModule_MultipleServices_GroupsCorrectly(CancellationToken token)
    {
        var services = new[]
        {
            new StateServiceInfo("global::Test.ModuleA", "ModuleA", "global::Test.ServiceA_Factory"),
            new StateServiceInfo("global::Test.ModuleA", "ModuleA", "global::Test.ServiceB_Factory"),
            new StateServiceInfo("global::Test.ModuleB", "ModuleB", "global::Test.ServiceC_Factory")
        };

        var groups = StateModuleServiceGenerator.GroupServicesByModule(services).ToArray();

        await Assert.That(groups).HasCount().EqualTo(2);

        var groupA = groups.First(g => g.ModuleTypeName == "ModuleA");
        await Assert.That(groupA.ServiceFactories).HasCount().EqualTo(2);
        await Assert.That(groupA.ServiceFactories[0].FactoryTypeName).IsEqualTo("global::Test.ServiceA_Factory");
        await Assert.That(groupA.ServiceFactories[1].FactoryTypeName).IsEqualTo("global::Test.ServiceB_Factory");

        var groupB = groups.First(g => g.ModuleTypeName == "ModuleB");
        await Assert.That(groupB.ServiceFactories).HasCount().EqualTo(1);
        await Assert.That(groupB.ServiceFactories[0].FactoryTypeName).IsEqualTo("global::Test.ServiceC_Factory");
    }
}
