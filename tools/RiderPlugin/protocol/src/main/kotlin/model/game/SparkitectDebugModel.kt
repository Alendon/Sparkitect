...
// The game channel (D-13): a standalone Root() the engine hosts over a SocketWire.Server and the
...
// construction (proven in plan 01). NOT rooted on SolutionModel: the frontend cannot socket the
// game (Pitfall 5); the backend republishes to the frontend via DebugToolWindowModel.
//
// It references the shared DebugLibrary structdefs (single data design) rather than declaring its
// own, and layers the typed GSM view over the D-21 generic entry floor so a mod can later bind its
...
object SparkitectDebugModel : Root() {
    ...
    init {
        // Typed GSM view (D-22): the current composition snapshot, pushed on connect and on every
        // composition change (D-06). Nullable = not yet published (pre-first-compose).
        property("snapshot", DebugLibrary.DebugSnapshot.nullable)

        // D-21 generic self-describing floor: a string-keyed set of entries for schema-less
