...
// The backend -> frontend republish. An Ext on the Solution model, bound by the
...
object DebugToolWindowModel : Ext(SolutionModel.Solution) {
    ...
    // A discovered Sparkitect process the selector offers. The backend's DiscoveryWatcher owns
    ...
    // backend is the sole game-channel client) — the frontend shows pid + version.
    val ProcessInfo = structdef {
        field("pid", int)
        field("engineVersion", string)
    }

    // Which source target a tree row navigates to: double-click resolves the type declaration,
    ...
    init {
        ...
        // The process (pid) whose channel the window currently shows. Nullable = no process
        // selected; when a debug session is active the backend defaults it to that process.
        property("selectedProcess", int.nullable)

        // The live discovered-process list the selector renders. Backend -> frontend: the
        ...
        // Row navigation. The frontend calls with a (row-id, target); the backend runs the
