...
// rdgen cross-process debug channel. One shared structdef library (model.lib.DebugLibrary)
...
// across every endpoint — the frontend cannot socket the game directly, so the backend republishes.
plugins {
...
//   frontend — Rider frontend (Kotlin); consumes the ext republish + lib. NOT the game Root — the
//              frontend cannot socket the game directly.
...
// emitted separately and compiled by the plugin build, not this standalone subproject.
...
sourceSets {
    ...
    // republish Kotlin binds against the full Rider platform and is compiled by the plugin build.
    test {
