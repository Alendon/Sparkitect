...
/// <summary>
/// A composite WASD-shaped keyboard binding type (D-16): composes four keys' sampled
/// pressed-states into an <see cref="ActionResult{T}"/> of <see cref="Vector2"/>. No key pressed
/// produces <see cref="ActionResult{T}.NoValue"/>, NEVER <c>Value(Vector2.Zero)</c> — "produces a
/// value" means "actively contributing" (D-19); "stick centered" and "nothing contributing" are
/// different facts. Backed by <see cref="KeyboardVector2Setting"/>, ONE composite value-type
/// setting for all four keys — the codebase's FIRST composite value-type setting (R-6).
/// </summary>
/// <remarks>
...
/// fields are raw sampled channel slots, written by the owning <see cref="KeyboardSourceProvider"/>'s
/// bulk fill before <see cref="Evaluate"/> runs — this type never samples the device itself.
/// </remarks>
[PublicAPI]
public readonly struct KeyboardVector2 : IBindingType<KeyboardVector2, Vector2>
{
    ...
    public KeyboardVector2Setting Setting { get; }
    ...
    public KeyboardVector2(
        KeyboardVector2Setting setting,
