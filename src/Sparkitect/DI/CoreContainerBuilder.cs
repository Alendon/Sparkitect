...
internal class CoreContainerBuilder : ICoreContainerBuilder
{
    ...
    public ICoreContainerBuilder Register(IServiceFactory serviceFactory)
    {
    ...
    public ICoreContainerBuilder Override(IServiceFactory serviceFactory)
    {
        var serviceType = serviceFactory.ServiceType;
    ...
    {
        return _instances.TryGetValue(serviceType, out instance);
    }
    ...
    private void ValidateDependencyGraph()
    {
        ...
        foreach (var serviceFactory in _registrations.Values)
        {
            ...
            foreach (var (dependencyType, propertyName, isOptional) in serviceFactory.GetPropertyDependencies())
            {
                if (isOptional)
                    continue;
